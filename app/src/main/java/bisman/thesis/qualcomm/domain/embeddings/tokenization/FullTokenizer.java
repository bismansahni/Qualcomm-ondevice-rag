...
public class FullTokenizer {
    ...
    public FullTokenizer(Map<String, Integer> vocab, boolean doLowerCase) {
        this.vocab = vocab;
        this.basicTokenizer = new BasicTokenizer(doLowerCase);
        this.wordpieceTokenizer = new WordpieceTokenizer(vocab, "[UNK]", 200);
    }
    ...
    public List<Integer> convertTokensToIds(List<String> tokens) {
        ...
        for (String token : tokens) {
            outputIds.add(vocab.getOrDefault(token, vocab.get("[UNK]")));
        }
        return outputIds;
    }
}